...
public class ConfigListEntryValue {
	...
	public ConfigListEntryValue(ConfigListEntry entry, int value) {
		this.entry = entry;
		this.regValue = value;
	...
	public ConfigListEntry getEntry() {
		return entry;
	}
	...
	public String getDescription() {
		...
		if (!entry.conversion.equals(""))
			try {
				setting = ConfigLookups.getSetting2Value(entry.conversion, value);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public String getConfigValue(String configName) {
		...
		if (!entry.conversion.equals(""))
			try {
				setting = ConfigLookups.getSetting2Value(entry.conversion, iValue);
			} catch (Exception e) {
				e.printStackTrace();
				setting = e.getClass().getSimpleName();
			}
